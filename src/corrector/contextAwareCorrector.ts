...
const ALLOWED_SAI_VERBS = ["nhận", "xây", "mua", "setup", "con", "trên", "chạy", "chăm", "quản lý", "chuyển", "mở"];
...
export class ContextAwareTranscriptCorrector implements TranscriptCorrector {
  correct(input: string, context?: CorrectorContext): CorrectionResult {
    ...
    // 7. Context-Aware "sai" -> "site" with false-positive protection
    const hasForbiddenPattern = FALSE_POSITIVE_SAI_PATTERNS.some((pattern) => pattern.test(currentText));

    if (!hasForbiddenPattern) {
      // Check for verb + sai (e.g. "nhận sai", "xây sai")
      const verbPattern = new RegExp(`\\b(${ALLOWED_SAI_VERBS.join("|")})\\s+sai\\b`, "gi");
      currentText = currentText.replace(verbPattern, (match, verb: string) => {
        const replacement = `${verb} site`;
        changes.push({
          from: match,
          to: replacement,
          reason: "SEO domain context + phonetic similarity (sai -> site)",
          confidence: 0.94
        });
        return replacement;
      });

      // Check for modifier + sai (e.g. "sai mới", "sai betting", "sai iGaming")
      currentText = currentText.replace(/\bsai\s+(mới|betting|iGaming|vệ\s+tinh)\b/gi, (match, modifier: string) => {

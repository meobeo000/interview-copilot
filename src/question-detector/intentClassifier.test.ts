...
describe("Question Intent Classifier (Phase 1 Cleanup & Robust Multi-Signal Intent Scoring)", () => {
  ...
  describe("Phase 1 Multi-Signal Scoring without Hardcoded Phonetic Variants", () => {
    ...
    it("correctly routes non-monetary tactic question to STRATEGY_PLAN instead of BUDGET_ALLOCATION", () => {
      const transcript = "content entity pbn em triển khai thế nào";
      const res = classifyQuestionIntent(transcript);
    ...
    it("classifies domain comparison with DR & zero traffic as DOMAIN_SELECTION", () => {
      const transcript = "con A DR 55 traffic bằng không con B DR 20 có traffic thật em chọn con nào";
      const res = classifyQuestionIntent(transcript);
    ...
    it("classifies bot opening without keyword reception as NO_KEYWORD_SIGNAL", () => {
      const transcript = "site mở bot hai tuần rồi vẫn chưa nhận cây thì xử lý sao";
      const res = classifyQuestionIntent(transcript);
    ...
    it("classifies PBN timing question with spoken words as PBN_TIMING", () => {
      const transcript = "tại sao khoảng ngày thứ mười mới bắt đầu đi p b n";
      const res = classifyQuestionIntent(transcript);
  ...
  describe("Multi-Signal Scoring Diagnostics Breakdown", () => {
    it("produces structured score breakdowns for inspection", () => {
      ...
      expect(budgetScore).toBeDefined();
      expect(budgetScore?.totalScore).toBeGreaterThanOrEqual(10);
      expect(budgetScore?.signals.money).toBeDefined();
      expect(budgetScore?.signals.allocation).toBeDefined();
      expect(budgetScore?.signals.categories).toBeDefined();
    });
  });
});
